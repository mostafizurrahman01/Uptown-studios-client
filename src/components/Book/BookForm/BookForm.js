...
const BookForm = ({ modalIsOpen, closeModal, appointmentOn, date, information }) => {
    ...
    const onSubmit = data => {
        ...
        data.information = appointmentOn;
        data.date = date;
        ...
        fetch('http://localhost:7000/addPhotographerBook', {
            ...
            .then(success => {
                if (success) {
                    closeModal();
                    alert('Photographer Book Created Successfully');
                    history.push(`/book/${information._id}`);
